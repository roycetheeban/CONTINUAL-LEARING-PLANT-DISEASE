...
from common_catb import (
    ...
    save_logs, plot_training, save_confusion, dump_metrics, get_process_ram_mb
)


def train_epoch_new_only(model, loader, device, criterion, optimizer):
    model.train()
    total = 0.0
    for x, y in loader:
...
def main():
    ...
    for ep in range(1, int(cfg['train']['max_epochs'])+1):
        ...
        tr_loss = train_epoch_new_only(model, train_loader, device, criterion, optimizer)
    ...
    plot_training(logs, dirs['figures'] / 'train_val_curves.png', 'CatB Naive Training')
