...
def main():
    ...
    old_replay_ds = RemapImageFolder(cfg['data']['old_replay_dir'], eval_tfms, cls_to_idx)
    ...
    replay_loader = DataLoader(old_replay_ds, batch_size=int(cfg['train']['batch_size']), shuffle=True, num_workers=nw, pin_memory=True)
    ...
    plot_training(logs, dirs['figures'] / 'train_val_curves.png', 'CatB EWC Training')
    ...
    extra = {'fisher_batches_used': fisher_used, 'best_epoch': best['epoch']}
    dump_metrics(dirs, cfg, 'cat_b_ewc_head_expand', cfg['meta']['cycle_name'], global_classes, old_classes, new_classes, old_stats, new_stats, runtime, model, extra=extra)
